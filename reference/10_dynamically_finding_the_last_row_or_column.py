...
max_row = external_wb["books"].max_row
print(f"Furthest row with data: {max_row}")
external_wb["books"].auto_filter.ref = f'A1:D{max_row}'
...
# and print it out.
max_column  = external_wb.active.max_column
print(f"furthest column with data: {max_column}")
...
# Combine max column letter with the max row
external_wb["borrowers"].auto_filter.ref = f'A1:{column_letter}{max_row}'
external_wb.save(filename="example_10.xlsx")
external_wb.close()
